...
document.addEventListener('DOMContentLoaded', function () {
  ...
  console.log('[DEBUG] mobile-collapse encontrado:', !!mobileCollapse);
  if (mobileCollapse) {
    mobileCollapse.addEventListener('click', function (e) {
      e.preventDefault();
      var sidebar = document.querySelector('.pc-sidebar');
      console.log('[DEBUG] Clique no mobile-collapse. Sidebar encontrado:', !!sidebar);
      if (!sidebar) return;
      if (sidebar.classList.contains('mob-sidebar-active')) {
        console.log('[DEBUG] Removendo mob-sidebar-active');
        rm_menu();
      } else {
        console.log('[DEBUG] Adicionando mob-sidebar-active');
        ...
        overlay.onclick = function() {
          console.log('[DEBUG] Clique no overlay para fechar menu');
          rm_menu();
        };
        sidebar.appendChild(overlay);
      }
      // DEBUG: Verifica se a classe foi aplicada
      console.log('[DEBUG] Classes atuais da sidebar:', sidebar.className);
    });
  }

  // NOVO: Botão para esconder/exibir sidebar no desktop
  var sidebarHide = document.getElementById('sidebar-hide');
  console.log('[DEBUG] sidebar-hide encontrado:', !!sidebarHide);
  if (sidebarHide) {
    sidebarHide.addEventListener('click', function (e) {
      ...
      console.log('[DEBUG] Toggle pc-sidebar-hide. Classes atuais:', sidebar.className);
    });
  }
});
...
function rm_menu() {
  ...
  if (sidebar) {
    ...
    console.log('[DEBUG] Menu fechado, mob-sidebar-active removido');
  }
}
